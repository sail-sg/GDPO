...
class MolecularVisualization:
    ...
    def mol_from_graphs(self, node_list, adjacency_matrix):
        ...
        if self.dataset_infos.atom_decoder is None:
            atom_decoder = pyasl.AtomicNo().getElSymbol
        else:
    ...
    def visualize(self, path: str, molecules: list, num_molecules_to_visualize: int, log='graph'):
        ...
        for i in range(num_molecules_to_visualize):
            ...
            mol = self.mol_from_graphs(
                molecules[i][0].numpy(), molecules[i][1].numpy())
            try:
    ...
    def visualize_chain(self, path, nodes_list, adjacency_matrix, trainer=None):
        ...
        mols = [self.mol_from_graphs(nodes_list[i], adjacency_matrix[i])
        ...
        for frame in range(num_frams):
            ...
            Draw.MolToFile(mols[frame], file_name, size=(
                300, 300), legend=f"Frame {frame}")
            save_paths.append(file_name)

        imgs = [imageio.imread(fn) for fn in save_paths]
        gif_path = os.path.join(os.path.dirname(
            path), '{}.gif'.format(path.split('/')[-1]))
        imgs.extend([imgs[-1]] * 10)
        imageio.mimsave(gif_path, imgs, subrectangles=True, fps=5)

        if wandb.run:
            print(f"Saving {gif_path} to wandb")
            wandb.log({"chain": wandb.Video(
                gif_path, fps=5, format="gif")}, commit=True)

        # draw grid image
        try:
            img = Draw.MolsToGridImage(
                mols, molsPerRow=10, subImgSize=(200, 200))
            img.save(os.path.join(
                path, '{}_grid_image.png'.format(path.split('/')[-1])))
...
class NonMolecularVisualization:
    def to_networkx(self, node_list, adjacency_matrix):
        ...
        for i in range(len(node_list)):
            if node_list[i] == -1:
                continue
            graph.add_node(
                i, number=i, symbol=node_list[i], color_val=node_list[i])

        rows, cols = np.where(adjacency_matrix >= 1)
        edges = zip(rows.tolist(), cols.tolist())
        for edge in edges:
            edge_type = adjacency_matrix[edge[0]][edge[1]]
            graph.add_edge(edge[0], edge[1], color=float(
                edge_type), weight=3 * edge_type)
    ...
    def visualize(self, path: str, graphs: list, num_graphs_to_visualize: int, log='graph'):
        ...
        for i in range(num_graphs_to_visualize):
            ...
            graph = self.to_networkx(
                graphs[i][0].numpy(), graphs[i][1].numpy())
    ...
    def visualize_chain(self, path, nodes_list, adjacency_matrix):
        ...
        graphs = [self.to_networkx(nodes_list[i], adjacency_matrix[i])
                  for i in range(nodes_list.shape[0])]
        # find the coordinates of atoms in the final molecule
